...
public class SimpleHibernateDao<T, ID extends Serializable> {
	 ...
	 * 保存新增并返回实例化ID.
	 */
	public Serializable saveReturnId(final T entity) {
		AssertUtils.notNull(entity, "entity不能为空");
		logger.debug("save entity: {}", entity);
	...
	public  void delete(final ID[] ids) {
		...
		logger.debug("delete entity {},ids is {}", entityClass.getSimpleName(), ids);
	}
	...
	public List<T> get(final ID[] ids) {
		return find(Restrictions.in(getIdName(), ids));
	}
	...
	public long findMax(final String propertyName) {
		AssertUtils.hasText(propertyName, "propertyName不能为空");
		CriteriaImpl impl = (CriteriaImpl) createCriteria();
		Long maxObject = (Long) impl.setProjection(Projections.max(propertyName)).uniqueResult();
		long max = maxObject == null ? 0 : maxObject;
		return max;
	}
