...
function Dialog(content, options) {
    ...
    var isIe9= $.browser.msie && ('9.0' == $.browser.version);
    var isIe10 = $.browser.msie && ('10.0' == $.browser.version);

    
    /* 对话框的布局及标题内容。*/
    var barHtml = !options.showTitle ? '' :
        '<div class="bar" onselectstart="return false;" oncopy="return false;"><span class="title" onselectstart="return false;">' + options.title + '</span><a class="close" ' + ((options.showClose) ? '' : 'style="display:none;"') + '>' + options.closeText + '</a></div>';

    var dialog = $('<div id="' + options.id + '" class="dialog" style="width:' + options.dialogWidth + '">' + barHtml + '<div class="dialogcontent"></div></div>').hide();
    ...
    */
    var resetPos = function() {
        ...
        if (options.center) {
            ...
            if (!isIe6 && options.fixed) { dialog.css({ top: top, left: left }); }
    ...
    */
    var init = function() {
        ...
        if (options.modal) {
            $('body').append('<div id="' + overlayId + '" class="dialog-overlay" onselectstart="return false;" oncopy="return false;"></div>');
    ...
    this.show = function () {
        ...
        */
        var getOpacity = function (id) {
            if (!isIe || isIe9 || isIe10) { return $('#' + id).css('opacity'); }
            var opacity = 0.5;
            try {
                var el = document.getElementById(id);
                opacity = (undefined != el
        ...
        /* 是否显示背景遮罩层 */
        if (options.modal) { $('#' + overlayId).fadeTo('fast', getOpacity(overlayId)); }
    ...
    */
    this.hide = function() {
        ...
        if (options.modal) { $('#' + overlayId).fadeOut('fast'); }
        isShow = false;
    };
    ...
    */
    this.close = function() {
        ...
        $(window).unbind("resize", __onResize);
        dialog.fadeOut('fast', function() {
            $(this).remove();
            isShow = false;
            if (undefined != options.afterClose) { options.afterClose(); }
        });
        if (options.modal) { $('#' + overlayId).fadeOut('fast', function() { $(this).remove(); }); }
