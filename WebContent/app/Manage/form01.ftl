...
<div class="slide_right_tab">
	<div class="slide_screen">
		<form>
			<p>
				<span class="siyuannormal">车牌号码:</span>
				<span>
					<input id="cardNo" type="text" placeholder="请输入" class="text_input siyuannormal" style='background:url('');'/>
				</span>
				<span class="siyuannormal">所有人关键字:</span>
				<span>
					<input id="licenseName" type="text" placeholder="请输入" class="text_input siyuannormal" style='background:url('');'/>
					<input type="button" onclick="getPageList(1)" value="查询" class="btn_input" />
				</span>
				<span>
					<a href="javascript:void(0)" style="margin-left:5px; float:left;" id="search_pic" onclick="javascript:down('${base}/manage/Manage_exportExcel.do');">
	...
	<div class="slide_table" style=''>
		<table id="disTable" border="0" class="ranking_table">
			  <thead >
			    <tr>
			      <th>号牌号码</th><th>所有人</th><th>车辆识别代码</th><th>车辆类型</th><th>操作</th>
			    </tr>
			  </thead>
		</table>
		<!--分页-->
	...
	<tbody style="font-family:'微软雅黑';">
		{{each(i,item) result}}
			<tr style="font-family:'微软雅黑';">
                ...
                <td>{{= item.carType}}</td>
                <td>
				<a title="详情" href="javascript:void(0)" onclick="goDetail('{{= item.id}}')" class="btn_detail choice hover_a" ><img src="/cdl/images/cms/small_icon_10.png" style="border-radius:0; width:13px; height:13px;"></a>
                </td>
            </tr>
		{{/each}}
	<tbody>
...
function getOrderData(param){
        $.ajax({
            type: "post",
            url: "${base}/manage/Manage_ajaxLicenseList.do",
            data: param,
            dataType : 'json',
